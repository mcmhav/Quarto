...
public class N00bPlayer extends Player {
	public N00bPlayer(PlayerType pType, int id) {
		super(pType, id);
	}
	...
	protected Piece pickPiece(Board board, Scanner scanner, boolean doMiniMax){
		...
		if(playerType==PlayerType.RANDOM){
			...
			return p;
				...
				|| (playerType==PlayerType.MINIMAX4&&!doMiniMax)){
	...
	public void placePiece(Board board, Piece piece, Scanner scanner, boolean doMiniMax, boolean wrongPick){
		if(playerType==PlayerType.RANDOM){
			...
			Node2 node = this.miniMaxMove(3, board, piece);
			if(node==null){
			...
			Node2 node = this.miniMaxMove(4, board, piece);
			if(node==null){
				board.placePiece(board.getFreePlaces().get(0), piece);
			}
			else{
	...
	public Node2 miniMaxMove(int depth, Board board, Piece givenPiece){
		Board copy = new Board();
		copy.setBoard(board.getBoard());
		copy.setPieces(board.getPieces());
	
		Node2 root = new Node2(copy, true, null, givenPiece, true, 0);
		...
		if(root.isTerminal()){
			...
			Node2 best = root.getChildren().remove(0);
